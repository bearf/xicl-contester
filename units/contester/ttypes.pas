...
//   			_PC1.._PCMAX: Result := 'PC' + Chr(Ord(cResult) - Ord(_PC1) + Ord('1'));
   			_WA: begin Result := 'Wrong answer on test ' + IntToStr(cResult div 100); end;
   			_RE: begin Result := 'Runtime error on test ' + IntToStr(cResult div 100); end;
   			_TL: begin Result := 'Time limit exceeded on test ' + IntToStr(cResult div 100); end;
   			_ML: begin Result := 'Memory limit exceeded on test ' + IntToStr(cResult div 100); end;
   			_DL: begin Result := 'Waiting mode on test ' + IntToStr(cResult div 100); end;
   			_CE: Result := 'Compilation error';
   			_PE: begin Result := 'Presentation error on test ' + IntToStr(cResult div 100); end;
   			_FL: Result := 'Failure';
   			_BR: Result := 'Operator mode';
   			_NO: Result := 'Nothing :)';
  		else Result := '??'; end;
 	end;
